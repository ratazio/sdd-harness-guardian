...
Implementation requires a written spec with explicit `Outcome Ready: yes` and
`Spec Ready: yes` decisions. The spec is the source of truth for intent, not a
substitute for the technical plan.

## Required contract

The spec declares title, status, owner, problem, objective, delivery outcome,
actors, observable outcomes, non-goals, functional requirements, acceptance
criteria, edge cases, constraints, assumptions, risks, dependencies and
validation notes.
...
- product/user outcome, demonstrable increment or priority source is absent;
...
- the agent would need to infer commercial value or roadmap priority;
...
`spec.outcome_ready == true`, `spec.spec_ready == true` and the Spec Guardian
